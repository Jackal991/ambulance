...
local function GetAvailableBedprison(bedId)
    local pos = GetEntityCoords(PlayerPedId())
    local retval = nil
    if bedId == nil then
        for k, v in pairs(Config.Locations["bedsjail"]) do
            if not Config.Locations["bedsjail"][k].taken then
		if #(pos - vector3(Config.Locations["bedsjail"][k].coords.x, Config.Locations["bedsjail"][k].coords.y, Config.Locations["bedsjail"][k].coords.z)) < 500 then
                	retval = k
		end
            end
        end
    else
        if not Config.Locations["bedsjail"][bedId].taken then
		if #(pos - vector3(Config.Locations["bedsjail"][bedId].coords.x, Config.Locations["bedsjail"][bedId].coords.y, Config.Locations["bedsjail"][bedId].coords.z))  < 500 then
            		retval = bedId
		    end
        end
    end
...
local function GetAvailableBedpolice(bedId)
    local pos = GetEntityCoords(PlayerPedId())
    local retval = nil
    if bedId == nil then
        for k, v in pairs(Config.Locations["bedspd"]) do
            if not Config.Locations["bedspd"][k].taken then
		if #(pos - vector3(Config.Locations["bedspd"][k].coords.x, Config.Locations["bedspd"][k].coords.y, Config.Locations["bedspd"][k].coords.z)) < 500 then
                	retval = k
		end
            end
        end
    else
        if not Config.Locations["bedspd"][bedId].taken then
		if #(pos - vector3(Config.Locations["bedspd"][bedId].coords.x, Config.Locations["bedspd"][bedId].coords.y, Config.Locations["bedspd"][bedId].coords.z))  < 500 then
            		retval = bedId
		    end
        end
    end
...
local function GetAvailableBedsandy(bedId)
    ...
    local retval = nil
    if bedId == nil then
        for k, v in pairs(Config.Locations["sandy"]) do
            if not Config.Locations["sandy"][k].taken then
		if #(pos - vector3(Config.Locations["sandy"][k].coords.x, Config.Locations["sandy"][k].coords.y, Config.Locations["sandy"][k].coords.z)) < 500 then
                	retval = k
		end
            end
        end
    else
        if not Config.Locations["sandy"][bedId].taken then
		if #(pos - vector3(Config.Locations["sandy"][bedId].coords.x, Config.Locations["sandy"][bedId].coords.y, Config.Locations["sandy"][bedId].coords.z))  < 500 then
            		retval = bedId
		    end
        end
    end
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    TriggerEvent('nc-deathscreen:revive')
end)

RegisterNetEvent('hospital:client:ReviveBedprison', function()
    local player = PlayerPedId()
    ...
    TriggerServerEvent("hospital:server:SetDeathStatus", false)
    TriggerServerEvent("hospital:server:SetLaststandStatus", false)
    TriggerServerEvent("hospital:server:resetHungerThirst", "hunger", 80)
    TriggerServerEvent("hospital:server:resetHungerThirst", "thirst", 80)
...
RegisterNetEvent('hospital:client:ReviveBed', function()
    local player = PlayerPedId()
...
RegisterNetEvent('hospital:client:ReviveBedpolice', function()
    local player = PlayerPedId()
...
RegisterNetEvent('hospital:client:ReviveBedsandy', function()
    local player = PlayerPedId()
...
-- פונקציה שבודקת האם הנגע נמצא במים ומעניקה פצצה לשחקן
local function CheckWaterDamage(attacker, defender, weaponHash)
    local weaponName = GetHashKey(weaponHash)

    if weaponName == GetHashKey("weapon_stungun") and IsEntityInWater(attacker) then
        TriggerClientEvent('chatMessage', -1, "^1" .. GetPlayerName(defender) .. " התנפץ!") -- הודעה בצבע אדום לכולם
        SetEntityHealth(defender, 0) -- הפיכת ניקוד הבריאות של השחקן ל־0 (השחקן מת)
    end
end

-- פונקציה שבודקת האם הנגע נמצא במים ומעניקה פצצה לשחקן
local function CheckWaterDamage(attacker, defender, damageType, weaponHash)
    local weaponName = GetHashKey(weaponHash)

    if weaponName == GetHashKey("weapon_stungun") then
        local attackerPos = GetEntityCoords(attacker)
        local isAttackerInWater = IsPointInWater(attackerPos.x, attackerPos.y, attackerPos.z)

        if isAttackerInWater then
            TriggerClientEvent('chatMessage', -1, "^1" .. GetPlayerName(defender) .. "Death!") -- הודעה בצבע אדום לכולם
            SetEntityHealth(player, 0) -- הפיכת ניקוד הבריאות של השחקן ל־0 (השחקן מת)
        end
    end
end

-- פונקציה שמאזינה לאירוע של נפילת נשק במשחק
AddEventHandler('weaponDamageEvent', function(attacker, defender, damageType, weaponHash, isMelee)
    CheckWaterDamage(attacker, defender, damageType, weaponHash) -- קריאה לפונקציה שבודקת האם הנגע נמצא במים ומעניקה פצצה לשחקן
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        ...
        if isRevive then
            ...
            TriggerEvent("hospital:client:ReviveBed")
        else
            canLeaveBed = true
        end
    end)
...
RegisterNetEvent('hospital:client:SendToBedprison', function(id, data, isRevive)
    bedOccupying = id
    bedOccupyingData = data
    SetBedCam()
    CreateThread(function ()
        ...
        if isRevive then
            QBCore.Functions.Notify("You are being helped..")
            Wait(Config.AIHealTimer * 1000)
            TriggerEvent("hospital:client:ReviveBedprison")
        else
            canLeaveBed = true
        end
    end)
...
RegisterNetEvent('hospital:client:SendToBedpolice', function(id, data, isRevive)
    bedOccupying = id
    bedOccupyingData = data
    SetBedCam()
    CreateThread(function ()
        ...
        if isRevive then
            QBCore.Functions.Notify("You are being helped..")
            Wait(Config.AIHealTimer * 1000)
            TriggerEvent("hospital:client:ReviveBedpolice")
        else
            canLeaveBed = true
        end
    end)
...
RegisterNetEvent('hospital:client:SendToBedsandy', function(id, data, isRevive)
    ...
    CreateThread(function ()
        ...
        if isRevive then
            QBCore.Functions.Notify("You are being helped..")
            Wait(Config.AIHealTimer * 1000)
            TriggerEvent("hospital:client:ReviveBedsandy")
        else
            canLeaveBed = true
        end
    end)
...
RegisterNetEvent('hospital:client:SetBedprison', function(id, isTaken)
    Config.Locations["bedsjail"][id].taken = false
end)

RegisterNetEvent('hospital:client:SetBedpolice', function(id, isTaken)
    Config.Locations["bedspd"][id].taken = false
end)

RegisterNetEvent('hospital:client:SetBedsandy', function(id, isTaken)
    Config.Locations["sandy"][id].taken = false
end)
                ...
                if #(pos - checkins) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if doctorCount >= Config.MinimalDoctors then
                            ...
                            QBCore.Functions.Progressbar("hospital_checkin", "Checking in..", 6000, false, true, {
...
RegisterNetEvent("prison:CheckIn", function()
    if QBCore then
        local pos = GetEntityCoords(PlayerPedId())
    ...
    if doctorCount >= Config.MinimalDoctors then
        ...
        QBCore.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
            ...
            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
            local bedId = GetAvailableBedprison()
            if bedId then 
                TriggerServerEvent("hospital:server:SendToBedprison", bedId, true)
            else
                QBCore.Functions.Notify("Beds are occupied..", "error")
            end
        end, function() -- Cancel
...
RegisterNetEvent("Police:CheckIn", function()
    if QBCore then
        local pos = GetEntityCoords(PlayerPedId())

    if doctorCount >= Config.MinimalDoctors then
        TriggerServerEvent("hospital:server:SendDoctorAlertpolice")
    else
        TriggerEvent('animations:client:EmoteCommandStart', {"notepad"})
        QBCore.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, false, true, {
            disableMovement = true,
            ...
            TriggerEvent('animations:client:EmoteCommandStart', {"c"})
            local bedId = GetAvailableBedpolice()
            if bedId then 
                TriggerServerEvent("police:server:SendToBed", bedId, true)
            else
                QBCore.Functions.Notify("Beds are occupied..", "error")
            end
        end, function() -- Cancel
...
RegisterNetEvent("Sandy:CheckIn", function()
    if QBCore then
        local pos = GetEntityCoords(PlayerPedId())
    ...
    if doctorCount >= Config.MinimalDoctors then
        ...
        TriggerEvent('animations:client:EmoteCommandStart', {"notepad"})
        QBCore.Functions.Progressbar("hospital_checkinsandy", "Checking in..", 2000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if bedId then 
                TriggerServerEvent("sandy:server:SendToBed", bedId, true)
            else
                QBCore.Functions.Notify("Beds are occupied..", "error")
            end
        end, function() -- Cancel
...
RegisterNetEvent('ambulance:client:armory', function(data)
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "hospital", Config.Items)
end)

------ Ems stash ------
RegisterNetEvent('hospital:openEMSPersonal', function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "emsstash_"..QBCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "emsstash_"..QBCore.Functions.GetPlayerData().citizenid)
end)
